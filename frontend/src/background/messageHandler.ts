            ...
            try {
                ...
                switch (message.type) {
                    ...
                    case "PROFILE_URL_CHANGED": {
                        ...
                        const { username, userTag, profileInfo, currentUsername, currentUserTag } = message as {
                            type: string;
                            username: string;
                            userTag: string;
                            profileInfo: {
                        ...
                        console.log(`🖼️ PROFILE_URL_CHANGED 요청:`, { username, userTag, profileInfo, currentUsername, currentUserTag });
                        ...
                        try {
                            ...
                            const loggedInUsername = currentUsername || sessionState?.memexUsername;
                            const loggedInUserTag = currentUserTag || sessionState?.memexUserTag;
                            const isMyProfile = loggedInUsername === username && loggedInUserTag === userTag;

                            console.log(`🖼️ [PROFILE_URL_CHANGED] 프로필 소유자 확인:`, {
                                isMyProfile,
                                loggedIn: { username: loggedInUsername, userTag: loggedInUserTag },
                            ...
                            // 기존 프로필 정보 가져오기
                            const existingProfiles = await new Promise<Record<string, any>>((resolve) => {
                            ...
                            if (isMyProfile) {
                                ...
                                if (existingProfile &&
                                    ...
                                    console.log(`✅ [PROFILE_URL_CHANGED] 내 프로필 정보 이미 존재, 생략`);
                                    result = { success: true, data: { success: true, skipped: true } };
                                    break;
                                }
                                ...
                                await new Promise<void>((resolve, reject) => {
                                    storage.local.set({ [profileStorageKey]: existingProfiles }, () => {
                                        const runtime = browser?.runtime || (globalThis as any).chrome?.runtime;
                                        if (runtime?.lastError) {
                                            reject(new Error(runtime.lastError.message));
                                            return;
                                        }
                                        console.log("✅ [PROFILE_URL_CHANGED] 내 프로필 정보 local storage에 저장 완료");
                                        resolve();
                                    });
                                });
                                ...
                                await new Promise<void>((resolve, reject) => {
                                    storage.session.set({ squid_session_state: updatedState }, () => {
                                        const runtime = browser?.runtime || (globalThis as any).chrome?.runtime;
                                        if (runtime?.lastError) {
                                            reject(new Error(runtime.lastError.message));
                                            return;
                                        }
                                        console.log("✅ [PROFILE_URL_CHANGED] 내 프로필 정보 session에 저장 완료");
                                        resolve();
                                    });
                                });
                            } else {
                                ...
                                if (existingCache &&
                                    ...
                                    console.log(`✅ [PROFILE_URL_CHANGED] 다른 사람 프로필 캐시 존재, 생략`);
                                    result = { success: true, data: { success: true, skipped: true } };
                                    break;
                                }
                                ...
                                await new Promise<void>((resolve, reject) => {
                                    storage.local.set({ [profileStorageKey]: existingProfiles }, () => {
                                        ...
                                        console.log(`✅ [PROFILE_URL_CHANGED] 다른 사람 프로필 정보 local storage에 캐시 저장 완료`);
                                        resolve();
                                    });
                                });
                            }
                            ...
                            result = {
                                success: false,
                                error: error instanceof Error ? error.message : "프로필 정보 저장 실패",
                            };
                        }
                        break;
                    }
                    ...
                    case "FETCH_MEMEX_PROFILE_INFO": {
                        ...
                        try {
                            ...
                            if (memexTabs.length === 0 || !memexTabs[0]?.id) {
                                ...
                                result = { success: true, data: { profileImageUrl: null, tokenAddr: null, tokenSymbol: null, tokenImageUrl: null, memexWalletAddress: null } };
                                break;
                            }
                            ...
                            result = {
                                success: true,
                                data: { profileImageUrl: null, tokenAddr: null, tokenSymbol: null, tokenImageUrl: null, memexWalletAddress: null },
                            };
                        }
                        break;
                    }
