...
import {
  getCommentsFromBlockchain,
  type CommentAddedV2Event,
} from "../lib/contract/eventParser";
  ...
  useEffect(() => {
    ...
    const syncBlockchainComments = async () => {
      try {
        ...
        logger.info("블록체인 댓글 조회 완료", {
          gameId,
          count: blockchainComments.length,
        });
        ...
        // 현재 DB 댓글 (기준)
        const dbComments =
          queryClient.getQueryData<Comment[]>(["comments", gameId]) || [];
        ...
        if (missingComments.length > 0) {
          logger.info("블록체인에만 있는 댓글 보충", {
            count: missingComments.length,
          });
