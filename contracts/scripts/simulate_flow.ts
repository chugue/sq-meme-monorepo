...
async function main() {
  ...
  const TOKEN_ADDRESS = "REPLACE_WITH_TOKEN_ADDRESS";

  const factory = await hre.viem.getContractAt("GameFactory", FACTORY_ADDRESS);
  const token = await hre.viem.getContractAt("MockToken", TOKEN_ADDRESS);
  ...
  const gameAddress = logs[0].args.gameAddr;
  console.log("Game created at:", gameAddress);

  // 3. Add Comment
  const game = await hre.viem.getContractAt("CommentGame", gameAddress);
  ...
  // Approve
  const tokenAsUser1 = await hre.viem.getContractAt("MockToken", TOKEN_ADDRESS, { client: { wallet: user1 } });
  await tokenAsUser1.write.approve([gameAddress, parseEther("100")]);

  console.log("Adding comment...");
  const gameAsUser1 = await hre.viem.getContractAt("CommentGame", gameAddress, { client: { wallet: user1 } });
